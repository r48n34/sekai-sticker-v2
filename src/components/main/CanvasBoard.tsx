    ...
    try {
      ...
      const blobImage = await dataURLToBlob(
        stageRef.current!.toDataURL({
          width: Math.max(...stickerContent.map((v) => v.x + (v.width || 0))) || CONFIGS.stageWidth,
          height:
            Math.max(...stickerContent.map((v) => v.y + (v.height || 0))) || CONFIGS.stageHeight,
          pixelRatio: 1.1,
        }),
      );
              ...
              <Grid.Col span={{ base: 12, md: 6, lg: 8 }}>
                <Box>
                  <Card shadow="sm" padding="lg" radius="md" withBorder>
                    <Group justify="space-between" mb={16}>
                      ...
                      <Group>
                        <Tooltip label={t("Download PNG")}>
                          <ActionIcon variant="light" onClick={() => downloadCurrentPng()}>
                            <IconDownload style={{ width: "70%", height: "70%" }} stroke={1.5} />
                          </ActionIcon>
                        </Tooltip>

                        <Tooltip label={t("Copy PNG to clipboard")}>
                          <ActionIcon variant="light" onClick={() => copyCurrentPng()}>
                              ...
                              if (
                                ...
                                return (
                                  <CanvasTransImage
                                    ...
                                    onChange={(newAttrs: StickerObject) =>
                    ...
                    <Box mt={16}>
                      <SelectLayer data={stickerContent} selectCb={(id) => setSelectedId(id)} />
                    </Box>
                  </Card>
                            ...
                            {selectedShape.format === "text" && (
                              <>
                                ...
                                <Slider
                                  ...
                                  onChange={(value) => {
                                    const ind = stickerContent.findIndex(
                                      (v) => v.id === selectedId,
                                    );
